module.exports = function(config, sockjs, sessionFactory, redisPubSub, SocketDataHandler, logger) {
	var that = this;
		    ...
		    conn.once('data', function(sessionId) {
		    	var session = sessionFactory.getSession(sessionId, function(err, session) {
		    		if (err) {
		    			...
		    			logger.info("Connected to user via session", session);

		 				new SocketDataHandler(redisPubSub, session, conn, logger);
		    		}
		    	});
		    });
